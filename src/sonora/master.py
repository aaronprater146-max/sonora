...
def master(x: np.ndarray, target_lufs: float = -11.0, glue: float = 1.0,
    ...
    x = peak_eq(x.astype(FLOAT), 42.0, -2.5, 0.7).astype(FLOAT)
    x = peak_eq(x.astype(FLOAT), 130.0, 3.2 * punch, 0.8).astype(FLOAT)
    x = peak_eq(x.astype(FLOAT), 450.0, -2.0, 1.0).astype(FLOAT)
    x = peak_eq(x.astype(FLOAT), 3500.0, 2.6, 0.7).astype(FLOAT)
    ...
    # 7. limiter + loudness
    x = normalize_lufs(x, target_lufs, ceil_db)
...
def stem_fx(x: np.ndarray, hp: float | None = None, lp: float | None = None,
    ...
    if gain_db:
        x = (x.astype(FLOAT) * db2amp(gain_db)).astype(FLOAT)
    return x


def report(x: np.ndarray) -> str:
    x = ensure2(x)
    m, s = x[:, 0].astype(FLOAT), x[:, 1].astype(FLOAT)
